...
async def generate_key_fn(
    ...
    - tag_tpm_limit: Optional[dict] - key-specific per-request-tag tpm limit, keyed by request tag. Example - {"cell-1": 200000}. Each tag gets an independent counter; requests whose tag is absent fall back to the key-level tpm limit.
...
async def update_key_fn(
    ...
    - tag_tpm_limit: Optional[dict] - Per-request-tag TPM limits, keyed by request tag {"cell-1": 200000}. Each tag gets an independent counter; absent tags fall back to the key-level tpm limit.
