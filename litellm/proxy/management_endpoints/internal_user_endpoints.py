...
async def new_user(
    ...
    - tag_tpm_limit: Optional[dict] - Per-request-tag tpm limit, keyed by request tag {"cell-1": 200000}. Enforced for keys only; values set on a user are stored but not enforced per user.
