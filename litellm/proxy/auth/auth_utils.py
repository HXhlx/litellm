...
def get_key_tag_tpm_limit(
    ...
    """
    Get the per-request-tag tpm limit configured on a given api key.

    The returned dict is keyed by request tag, so each tag/group tracked on
    the key gets its own independent TPM counter.
    """
    if user_api_key_dict.metadata:
        return user_api_key_dict.metadata.get("tag_tpm_limit")
    return None
